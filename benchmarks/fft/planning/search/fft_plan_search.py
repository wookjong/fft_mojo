...
from planning.search.fft_cost_model import PlanMetrics, estimate_cost, estimate_metrics
from planning.execution.fft_plan_cooperative import worker_candidates_per_fft
...
class PlanChoices:
    ...
    # candidates (planning.execution.fft_plan_persistent.make_persistent_leaf_plan),
    ...
    # eventual renderer passes -- see planning.execution.fft_plan_lanes' own module
...
def _wrap_persistent_leaf_as_recursive_plan(
    ...
    kernel`, and `planning.diagnostics.spill_probe.probe_spill_free` vs. `probe_
...
def generate_lane_variant_candidates(
    ...
    leaf floored to `compute_lanes=1`) -- via `planning.execution.fft_plan_lanes.
...
def generate_tile_candidates(rows: int, cols: int, *, simd_lanes: int) -> list[tuple[int, int]]:
    ...
    win), but gives planning.diagnostics.spill_probe-driven search at least one
...
def generate_candidates(
    ...
    *,
    ...
    # This is the axis Phase 5 (planning.execution.fft_plan_lanes.py) built the
