"""Parses a `probe_pool_alignment.py`-driven sweep log (one `=== N=<n>
strategy=<s> ===` block per run, `[pool_align] ...` lines, then a
`cycles=... spill=... panic=...` summary line) and cross-checks each
observed runtime pool address against `planning.fft_unit_utilization`'s
...
from planning.target_profile import DEFAULT_TARGET_PROFILE as TARGET
